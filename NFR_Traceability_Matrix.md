    ...
    This table joins each quantified quality requirement (SRS) to the architectural tactic claimed to satisfy it (SAS), and the test/tool used to verify it. Grouped by [NFR category](non-functional_requirements.md) — one external tool per category typically covers several QR rows.

    ## Performance (tool: TBD)
    | ID | Quantified Requirement | Tactic in SAS | Test / Tool | Target / Actual |
    |----|------------------------|---------------|-------------|------------------|
    | QR-01 | P95 API response time under 2 seconds for all REST endpoints under normal load | Asynchronous task execution + fast acknowledgement (4.3 Performance) | TBD | <2s / TBD |
    | QR-02 | Phase 1 CTEM scan completes within 60 seconds for 90% of requests, bounded by slowest single OSINT lookup | Asynchronous aggregation of parallel OSINT providers (1, 4.3) | TBD | <60s / TBD |
